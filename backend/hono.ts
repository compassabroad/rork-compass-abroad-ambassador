...
`;
    ...
    console.log("[Setup] Step 1: Creating tables...");
    const tableResults = await dbQueryMultiple(DEFINE_TABLES_SQL);
    const tableErrors = tableResults.filter((r) => r.status !== "OK");
    results.push({ step: "tables", success: tableErrors.length === 0, message: `${tableResults.length} statements, ${tableErrors.length} errors` });

    // Step 2: Seed programs
    console.log("[Setup] Step 2: Seeding programs...");
    let programsExist = false;
    try {
      const existing = await dbQuery<{ id: string }>('SELECT id FROM programs LIMIT 1;');
      programsExist = existing.length > 0;
    } catch (_e) { /* table might not respond yet */ }

    if (!programsExist) {
      const SEED_PROGRAMS_SQL = `
CREATE programs:language_education SET name = 'Dil Eğitimi', name_en = 'Language Education', description = 'Yoğun dil eğitimi programları', default_commission_usd = 200, points = 80, icon = 'languages', countries = '["USA","UK","Malta","Ireland"]', duration = '2-24 hafta';
CREATE programs:bachelor SET name = 'Lisans', name_en = 'Bachelor', description = 'Lisans eğitimi programları', default_commission_usd = 800, points = 300, icon = 'graduation-cap', countries = '["USA","UK","Canada","Germany"]', duration = '4 yıl';
CREATE programs:high_school SET name = 'Lise Eğitimi', name_en = 'High School', description = 'Yurtdışı lise eğitimi', default_commission_usd = 600, points = 250, icon = 'school', countries = '["USA","UK","Canada"]', duration = '1 yıl';
CREATE programs:internship SET name = 'Staj', name_en = 'Internship', description = 'Uluslararası staj programları', default_commission_usd = 400, points = 160, icon = 'building', countries = '["USA","UK","Germany"]', duration = '3-12 ay';
CREATE programs:work_and_study SET name = 'Work and Study', name_en = 'Work and Study', description = 'Çalışma ve eğitim programı', default_commission_usd = 300, points = 120, icon = 'briefcase', countries = '["USA","Canada","Australia"]', duration = '6-12 ay';
CREATE programs:visa_consulting SET name = 'Vize Danışmanlığı', name_en = 'Visa Consulting', description = 'Vize başvuru danışmanlık hizmetleri', default_commission_usd = 150, points = 60, icon = 'file-text', countries = '["USA","UK","Canada","Schengen"]', duration = '1-3 ay';
CREATE programs:group_summer_school SET name = 'Grup Yaz Okulu', name_en = 'Group Summer School', description = 'Gruplar için yaz okulu programları', default_commission_usd = 250, points = 100, icon = 'users', countries = '["USA","UK","Malta"]', duration = '2-8 hafta';
CREATE programs:individual_summer_school SET name = 'Bireysel Yaz Okulu', name_en = 'Individual Summer School', description = 'Bireysel yaz okulu programları', default_commission_usd = 300, points = 120, icon = 'sun', countries = '["USA","UK","Malta"]', duration = '2-8 hafta';
CREATE programs:paid_teaching SET name = 'Maaşlı Öğretmenlik', name_en = 'Paid Teaching', description = 'Yurtdışında maaşlı öğretmenlik fırsatları', default_commission_usd = 350, points = 140, icon = 'book-open', countries = '["USA","UK","UAE","China"]', duration = '1-2 yıl';
CREATE programs:camp_usa SET name = 'Camp USA', name_en = 'Camp USA', description = 'Amerika\'da yaz kampı danışmanlığı programı', default_commission_usd = 250, points = 100, icon = 'tent', countries = '["USA"]', duration = '3-4 ay';
CREATE programs:canada_online_highschool SET name = 'Kanada Online Lise', name_en = 'Canada Online High School', description = 'Kanada akrediteli online lise eğitimi', default_commission_usd = 450, points = 180, icon = 'laptop', countries = '["Canada"]', duration = '1-4 yıl';
CREATE programs:canada_language SET name = 'Kanada Dil Eğitimi', name_en = 'Canada Language Education', description = 'Kanada\'da dil eğitimi programları', default_commission_usd = 200, points = 80, icon = 'maple-leaf', countries = '["Canada"]', duration = '2-24 hafta';
CREATE programs:masters SET name = 'Yüksek Lisans', name_en = 'Masters', description = 'Yüksek lisans programları', default_commission_usd = 1000, points = 400, icon = 'award', countries = '["USA","UK","Canada","Germany","Netherlands"]', duration = '1-2 yıl';
`;
      const progResults = await dbQueryMultiple(SEED_PROGRAMS_SQL);
      const progErrors = progResults.filter((r) => r.status !== "OK");
      results.push({ step: "programs", success: progErrors.length === 0, message: `${progResults.length} statements, ${progErrors.length} errors` });
    } else {
      results.push({ step: "programs", success: true, message: "Already seeded" });
    }

    // Step 3: Seed ambassadors
    console.log("[Setup] Step 3: Seeding ambassadors...");
    let adminExists = false;
    try {
      const existing = await dbQuery<{ id: string }>("SELECT id FROM ambassadors WHERE email = 'admin@compassabroad.com' LIMIT 1;");
...
`;
      const ambResults = await dbQueryMultiple(SEED_AMBASSADORS_SQL);
      const ambErrors = ambResults.filter((r) => r.status !== "OK");
      results.push({ step: "ambassadors", success: ambErrors.length === 0, message: `${ambResults.length} statements, ${ambErrors.length} errors` });
    } else {
      results.push({ step: "ambassadors", success: true, message: "Already seeded" });
    }

    const allSuccess = results.every((r) => r.success);
    console.log("[Setup] Complete. All success:", allSuccess, results);
    return c.json({ success: allSuccess, results });
  } catch (e) {
    console.error("[Setup] Fatal error:", e);
    return c.json({ success: false, error: String(e), results });
  }
});
